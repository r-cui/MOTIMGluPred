# https://github.com/r-cui/GluPred/tree/master/model
...
def reselfattn(horizon_len: int):
    return ReSelfAttn(horizon_len)
...
class ReSelfAttn(nn.Module):
    def __init__(self, horizon_len: int):
        super(ReSelfAttn, self).__init__()
        self.horizon_len = horizon_len
        self.emb = nn.Linear(1, 128)
        self.encoder = Encoder(
            num_layers=3, d_model=128, heads=4, d_ff=512, dropout=0.1, attention_dropout=0.1
        )
    ...
    def forward(self, x: Tensor, x_time: Tensor, y_time: Tensor) -> Tensor:
        ...
        for _ in range(self.horizon_len):
            ...
            y_hat = self._encoder_forward(x_clone)
            x[:, :-1] = x_clone[:, 1:]
            x[:, -1] = y_hat[:, -1]
            res.append(y_hat[:, -1])
